...
def parse_args():
    ...
    parser.add_argument('--model', default='dibxmnist', type=str, help='model choice: dibx, dibx-mnist, vibi, vibi-mnist, gradcam, lime')
    ############################################################################################
    parser.add_argument('--data', default='mnist', type=str, help='dataset name: echograms, seal')
    parser.add_argument('--explainer_in_depth', default=1, type = int, help='explainer input depth')
    parser.add_argument('--explainer_out_depth', default=1, type = int, help='explainer output depth')
    parser.add_argument('--nmb_category', default=10, type = int, help='number of classes')
    ############################################################################################
    parser.add_argument('--pretrain_epoch', default=10, type = int, help='pretrain the classifier before end-to-end training')
    ...
    parser.add_argument('--dibx_gamma', default=0, type = float,
                        help = 'gamma regulating KLDloss btw learned mask and and its prior')
    parser.add_argument('--dibx_kernel_size', default=6, type = int, help = 'kernel size for DIB:entropy')
    parser.add_argument('--dibx_MI_patch_split', type=int, default=1, help='the patch split count for MI(T,X), if 4 , patch_size:32')
    ############################################################################################
    ...
    ############################################################################################
    parser.add_argument('--save_accuracy', default=0.8, type = float, help = 'minumum prediction accuracy for save_checkpoint_function')
    parser.add_argument('--bn', default=False, type=str2bool, help='batch-norm-layer')
    parser.add_argument('--chunk_size', default=4, type = int, help = 'superpixel dim for explainability')
